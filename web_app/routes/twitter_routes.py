...
from web_app.services.basilica_service import api as basilica_api_client
...
def fetch_user(screen_name=None):
    ...
    # return jsonify({"user": user._json, "tweets":
    ...
    for index, status in enumerate(tweets):
        ...
        # todo: prefer to make a single request to basilica with
        # all the tweet texts, instead of a request per tweet
